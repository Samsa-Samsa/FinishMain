...
namespace FinishMenuPrefab.Scripts.GameFinishPrefab
{
    ...
    public class ParticleSpawner : MonoBehaviour
    {
        ...
        private void InstantiateParticle()
        {
            ...
            for (var i = 0; i < particleSpawnPoints.Count; i++)
            {
                var particle = Instantiate(particlePrefab, particleSpawnPoints[i], Quaternion.identity);
                particle.transform.localScale /= 2f; // Reduces the particle size by half.
